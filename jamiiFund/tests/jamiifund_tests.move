...
fun test_create_group() {
    ...
    test_scenario::next_tx(&mut scenario, ADMIN);
    {
        let mut jamiifund = test_scenario::take_shared<JamiiFund>(&scenario);
        let lock = clock::create_for_testing(test_scenario::ctx(&mut scenario));

        // Create a new group
        core::create_group(
            &mut jamiifund,
            string::utf8(b"Test Savings Group"),
            MIN_CONTRIBUTION,
            LOAN_RATIO,
            &clock,
        ...
        test_scenario::return_(jamiifund);
    }